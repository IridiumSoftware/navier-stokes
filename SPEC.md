...
**v0.15.0 (2026-06-12).** Every entry (prose form; the pipes here are the field *schema*, not a literal layout): `NS-ID | Class | Statement | Evidence |
...
α-continuation boundary instrument (grok-test GO-023/024; `:open`, Scope ≠ PDE). `:proved` = 0. (39 entries.)
...
- **Witness-lane state (2026-06-12, the DNS integral-proxy lane is CLOSED):** pointwise domination
  non-uniform (uniform-domination probe) → integral form triad-trimmed to weight-sensitive +
  **FEED-marginal** (`R_feed≈0.98–1.03`; `ns046_integral_cancellation_probe`, the sign Required-Check
  closed algebraically) → shells/CKN/soft-weights STRESSED (grok GO-001/010/012, verified) → **the
  composed probe (shell-localized R_feed, grok GO-022) met its pre-stated kill N-stably 64↔128 ⇒ the
  Hole-A DNS lane is FULLY capped — zero licensed integral-proxy probes remain in either map.** What
  remains here is exactly the analytic target above, measured against the FEED `¼(|ω|²−(ω·e₃)²)`.
  Details: `docs/ns046_target.md` + `docs/gomap_verification_2026-06-12.md` (addendum).
...
**NS-050 — Modulation theory for the Type-II frontier.** *FORWARD-TARGET* (frontier map + within-truncation witness). Evidence: external-spec-reference (prior art, C1) + computed witness (1D-model). **Status: :open. Scope: PDE-analysis (≠ the PDE).** Source: `docs/ns050_modulation_type2_scope.md` (+ `ns050_dss_modulation.md`, `ns050_modulation_witness_companion.md`); witness `scripts/ns050_modulation_witness.jl` (+.out.txt); **instrument+tooling arc** `scripts/ns050_{dss_spectral_gap,twoscale_fit,3d_control,boussinesq_2d,boussinesq_wall,houluo_hl,houluo_profile,houluo_profile_solve,houluo_newton,houluo_newton_seeded,mapped_grid}.jl` (+.out.txt) + companions `docs/ns050_{dss_spectral_gap,twoscale_and_control,boussinesq_2d,boussinesq_wall,houluo_hl,houluo_profile}_companion.md` + `docs/ns050_mapped_grid_solver_scope.md` (see the Instrument-arc note below; T-25/T-26). **Attempt 5 (2026-06-13, stabilized dynamic rescaling, T-32):** modulation-pinned (c_l,μ) τ-evolution on the T-25 operators — the CLM GATE PASSES (recovers Φ=−4η/(1+4η²), (μ,c_l)=(1,1), residual ~1e-11, seed-independent ⇒ a validated profile solver), but the HL solve is a CLEAN NEGATIVE: it diverges from both seeds (monotone wrong-way drift, robust across dτ/N + a G-renorm variant) ⇒ the HL profile is a SADDLE the modulation cannot stabilize (brief §0; needs Newton/unstable-mode projection, Chen–Hou). Profile reconstruction stays OPEN (now 5 honest negatives); `scripts/ns050_dynrescale_profile.jl` + `ns050_hl_diag.jl` (+.out.txt) + `docs/ns050_dynrescale_profile_companion.md`. `:proved`=0; distance UNTOUCHED.
...
**Kill criteria (merged):** the seed's two stand (the d-anchor falls if Hou's continuation proves a pure regularizer — *the 1D-proxy half is already met*; the α-family reduces to citation+port if its boundary mechanism is found characterized in the literature); plus Grok's GO-024 watch (α-boundary indistinguishable from regular interior). Next rungs: α-dial on Hou–Luo (HQWW profiles); the genuine d-dial on the `(r,z)` solver; ~~a verify-pass on GO-023/024 per A7~~ (done — round 2, byte-identical). Depends NS-002/007/048/050; the Liouville-map relation (`docs/ns048_ancient_liouville_litmap.md` §5): the same monotonicity-formula hunt from the construction side. Distance UNTOUCHED.

---

**NS-054 — CFS admissibility on the MDAGC conjunction (pre-registered; the kill fired).** *POSSIBILISTIC / FORWARD-TARGET (attack-reduction, closed).* Evidence: argued (the paper pass; pre-registration + RWC-054 + over-reach pre-declaration adopted from Aaron's stub, `~/Desktop/NS-054_stub.md`). **Status: :falsified (as an attack-reduction — the pre-registered kill criterion fired; not a failure: it formally closes the move). Scope: methodology** (no PDE claim; the executable layer never produced one). Source: `docs/ns054_admissibility_companion.md` (+ MDAGC §5b). `:proved`=0.

The one unrun application of the CFS/inverse-Born admissibility lens (NS-037 machinery): point it at the **generator-class conjunction** `G1∧…∧G5` and ask whether the class carries a **joint obstruction** beyond its conjuncts. The stub pre-reduced the move (MDAGC §6 already gives "A empty ⟺ NS-046∧NS-048 closed") to one question — *a joint obstruction among {G1,G2,G5}×{G3,G4} beyond the used G3∧G4 coupling?* — with a pre-registered kill. **The paper pass (all six cross-pairs) found none:** every real cross-edge is existing literature (G1∧G4 = the sequential-`L³` backward-wildness of the ancient limit, *tier-pending*; G2∧G4 = CKN-ε inside KNSS's compactness, already silently used; G2∧G5 = the energy lane's exact ceiling; the rest internal/none) — the G3-pattern confirmed: *membership is bought with hard analysis; the lens cannot manufacture it.* **Verdict: "is A empty?" = NS-048(+NS-046), relabeled — the predicted exit, formally closed.** Residual banked: the **four-edge coupling inventory** (MDAGC §5b), whose one open-tier edge (G1∧G4) sharpens the litmap §4.1 line-read priority. Caveat: this enumeration is not a nonexistence proof of a joint obstruction; any return clears RWC-054 (joint-not-single; hard-not-instantaneous; not-a-relabeling, 3-seat witnessed; emptiness ⇒ machine-checked). Depends `docs/ns_blowup_generator_class.md`, NS-037, NS-048, NS-046, NS-007, NS-024/013 (the relabeling precedents), NS-031. Distance UNTOUCHED.
