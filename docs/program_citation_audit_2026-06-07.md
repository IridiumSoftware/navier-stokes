...
| 2 | **NRŠ Acta 1996 + Tsai ARMA 1998** (NS-007) — self-similar exclusion | **NRŠ C3** ✅ *(Acta primary line-read 2026-06-07, `docs/nrs_ess_verification_2026-06-07.md`)* · **Tsai C3** | A **global no-go** (kills the backward self-similar construction) | **Global** | **DONE** — `L³` faithful (Thm 1, p.291; local-energy explicitly open = Tsai's domain) |
...
| 5 | **Pan–Li, NA:RWA 56 (2020)** (arXiv:1908.11591) — **NO-SWIRL** sublinear-growth Liouville (`α<1`; `α=1` sharp, swirl-free counterexamples) | **C3** ✅ *(line-read 2026-06-07)* | ~~route-(i) counterexample suspicion~~ **withdrawn** (Pan–Li is no-swirl); pins the weak-radial *with-swirl* regime as **open** | local | **DONE** — corrected "swirl allowed"→no-swirl; venue→NA:RWA |
| 6 | **ESS 2003** (NS-005) `L_{3,∞}` endpoint + backward-uniqueness | **C3** ✅ *(proof line-read 2026-06-07 via the authors' verbatim English version; published-RMS `L_{3,∞}` metadata visually confirmed)* | The critical-`L_{3,∞}` endpoint; "singularity ⇒ `‖u‖_{L³}→∞`"; M6 | **Global** | **DONE** — space is weak-`L³`; blow-up + Carleman structure confirmed |
