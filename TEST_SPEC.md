...
| **T-32** NS-050 (stabilized dynamic-rescaling profile solve, attempt 5) | closed-form GATE + honest-negative: the modulation-pinned (c_l,μ) τ-evolution on the T-25 cot-map operators recovers the EXACT CLM self-similar profile Φ=−4η/(1+4η²) with eigenvalues (μ,c_l)=(1,1) to residual ~1e-11 and shape ~1e-5, from TWO independent seeds (seed-independent). The IDENTICAL machinery on HL DIVERGES from both seeds — diagnostic (`scripts/ns050_hl_diag.jl`) shows a monotone wrong-way drift (μ↛1, c_l↛+2.5) robust across dτ {1e-5,2e-6}, N {1024,2048}, and a G-renorm variant ⇒ the HL profile is a SADDLE the modulation cannot stabilize (the brief's pre-registered §0 negative; Chen–Hou's Newton+stability-control is what it needs). `scripts/ns050_dynrescale_profile.jl` (+.out.txt) + companion `docs/ns050_dynrescale_profile_companion.md`. | GATE PASS / HL clean-negative, **Scope: 1D-model / numerical-tooling (NOT NS, NOT PDE)** |
